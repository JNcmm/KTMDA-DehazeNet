...
class dehaze_net(nn.Module):
    def __init__(self, depth_rate=16, kernel_size=3, num_dense_layer=4, growth_rate=16, res_blocks=6):
        ...
        self.rdb_in = RDB(3, num_dense_layer, 3)
        self.rdb_in1 = RDB1(6, num_dense_layer, 6)
        self.ca1 = CALayer(16)
        self.palayer1 = PALayer(16)
        self.down1 = Down()
        self.up1 = UpsampleConvLayer(3, 3, 3, 2)
        self.relu = nn.ReLU(inplace=True)
        self.e_conv1 = nn.Conv2d(6, 6, 1, 1, 0, bias=True)
        self.e_conv2 = nn.Conv2d(6, 6, 3, 1, 1, bias=True)
        self.e_conv3 = nn.Conv2d(12, 6, 5, 1, 2, bias=True)
        self.e_conv4 = nn.Conv2d(12, 6, 7, 1, 3, bias=True)
        self.e_conv5 = nn.Conv2d(24, 6, 3, 1, 1, bias=True)
        self.e_conv6 = nn.Conv2d(6, 3, 1, 1, 0, bias=True)

        self.e_conv = nn.Conv2d(3, 16, 3, 1, 1, bias=True)
        self.e_conv10 = nn.Conv2d(16, 64, 3, 1, 1, bias=True)
        self.e_conv9 = nn.Conv2d(3, 3, 3, 1, 1, bias=True)
        self.e_conv8 = nn.Conv2d(3, 3, 3, 1, 1, bias=True)
        self.e_conv7 = nn.Conv2d(64, 16, 3, 1, 1, bias=True)
        self.e_conv11 = nn.Conv2d(16, 3, 3, 1, 1, bias=True)
        ...
        self.dehaze2 = nn.Sequential()
        for i in range(1, res_blocks):
            self.dehaze2.add_module('res%d' % i, ResidualBlock(12))
        self.dehaze3 = nn.Sequential()
        for i in range(1, res_blocks):
            self.dehaze3.add_module('res%d' % i, ResidualBlock(24))
