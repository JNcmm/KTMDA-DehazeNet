...
def populate_train_list(orig_images_path, hazy_images_path, rdb_path, gama_path, zft_path):
    train_list = []
    val_list = []
    ...
    for key in list(tmp_dict.keys()):
        ...
        if key in train_keys:
            for hazy_image in tmp_dict[key]:
                train_list.append([orig_images_path + key, hazy_images_path + hazy_image, rdb_path + key, gama_path + key, zft_path + key])


        else:
            for hazy_image in tmp_dict[key]:
                val_list.append([orig_images_path + key, hazy_images_path + hazy_image, rdb_path + key, gama_path + key,  zft_path + key])
...
class dehazing_loader(data.Dataset):
    ...
    def __init__(self, orig_images_path, hazy_images_path, rdb_path,gama_path,zft_path, mode='train'):

        self.train_list, self.val_list = populate_train_list(orig_images_path, hazy_images_path, rdb_path, gama_path, zft_path)
    ...
    def __getitem__(self, index):
        ...
        data_orig_path, data_hazy_path, rdb_path, gama_path, zft_path = self.data_list[index]
        ...
        return data_orig.permute(2, 0, 1), data_hazy.permute(2, 0, 1), rdb.permute(2,0,1), gama.permute(2, 0, 1), zft.permute(2, 0, 1)
