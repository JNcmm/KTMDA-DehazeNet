...
import argparse
import time
import KTMDA.teacher_model.newdataloader
import numpy as np
...
import KTMDA.teacher_model.RDBCAPAnet
...
def train(config):
    ...
    dehaze_net = KTMDA.teacher_model.RDBCAPAnet.dehaze_net().cuda()
    ...
    #                                               config.zft_path,
    #                                               config.white_path)
    train_dataset = KTMDA.teacher_model.newdataloader.dehazing_loader(config.orig_images_path,
                                               config.hazy_images_path)
    ...
    for epoch in range(config.num_epochs):
        ...
        for iteration, (img_orig, img_haze) in enumerate(train_loader):
            ...
            clean_image,f1 = dehaze_net(img_haze)
...
if __name__ == "__main__":
    ...
    b=1
    #看不出对应数据的可以参考test2
    parser.add_argument('--orig_images_path', type=str, default="D:/ws/KTMDA-DehazeNet-main/KTMDA/outdoor/train/clear/")
    parser.add_argument('--hazy_images_path', type=str, default="D:/ws/KTMDA-DehazeNet-main/KTMDA/outdoor/train/clear/")
