...
class dehaze_net(nn.Module):
    def __init__(self, depth_rate=16, kernel_size=3, num_dense_layer=4, growth_rate=16, res_blocks=6):
        ...
        self.rdb_in = RDB(9, num_dense_layer, 9)
        # self.rdb_in = RDB(3, num_dense_layer, 9)
        self.rdb_in1 = RDB1(18, num_dense_layer, 18)
        self.ca1 = CALayer(16)
        self.palayer1 = PALayer(16)
        self.down1 = Down()
        self.up1 = UpsampleConvLayer(9, 9, 3, 2)
        self.relu = nn.ReLU(inplace=True)
        self.e_conv1 = nn.Conv2d(18, 18, 1, 1, 0, bias=True)
        self.e_conv2 = nn.Conv2d(18, 18, 3, 1, 1, bias=True)
        self.e_conv3 = nn.Conv2d(36, 18, 5, 1, 2, bias=True)
        self.e_conv4 = nn.Conv2d(36, 18, 7, 1, 3, bias=True)
        self.e_conv5 = nn.Conv2d(72, 18, 3, 1, 1, bias=True)
        self.e_conv6 = nn.Conv2d(18, 9, 1, 1, 0, bias=True)

        self.e_conv = nn.Conv2d(9, 16, 3, 1, 1, bias=True)
        # self.e_conv = nn.Conv2d(3, 16, 3, 1, 1, bias=True)

        self.e_conv10 = nn.Conv2d(16, 64, 3, 1, 1, bias=True)
        self.e_conv9 = nn.Conv2d(9, 3, 3, 1, 1, bias=True)
        self.e_conv8 = nn.Conv2d(9, 9, 3, 1, 1, bias=True)
        self.e_conv7 = nn.Conv2d(64, 16, 3, 1, 1, bias=True)
        self.e_conv11 = nn.Conv2d(16, 9, 3, 1, 1, bias=True)
        ...
        self.dehaze3 = nn.Sequential()
        for i in range(1, res_blocks):
            self.dehaze3.add_module('res%d' % i, ResidualBlock(72))
        ...
        # Initial
        self.fu1.data.fill_(0.3)
        self.fu2.data.fill_(0.3)
        self.fu3.data.fill_(0.4)
        self.conv = nn.Conv2d(3,9,3,1,1)

    # def forward(self, x, y, z):
    def forward(self, x):

        # l = self.fu1 * x + self.fu2 * y + self.fu3 * z
        # l = self.relu(self.e_conv(l))
        # y = y.cpu().detach().numpy()
        x = x.cpu().detach().numpy()
        # z = z.cpu().detach().numpy()
        l = np.concatenate((x, x, x), axis=1)
